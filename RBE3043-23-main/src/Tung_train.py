...
class Trainer:
    ...
    def my_train(self, batch_size=32):
        num_episodes = 1000
        replay_buffer = []  # Buffer to store the transitions (state, action, reward, next_state)
        epsilon = 0.9

        for episode in range(num_episodes):
            state, _ = self.env.reset()  # Initialize the environment
            ...
            while not done:
                # Step 1: Generate action parameters and evaluate Q-values
                x_params = self.X_net(state)  # (num_actions, param_dim)
                print("x_params", x_params)
                q_values = self.Q_net(state, x_params)  # (num_actions)

                # Step 2: Select discrete action k* (epsilon-greedy)
                if random.random() < epsilon:
                    d_action = random.choice(range(self.Decrete_action_dim))  # Random action (exploration)
                else:
                    d_action = torch.argmax(q_values).item()  # Best action (exploitation)

                # Extract parameters for the chosen action
                c_action_params = x_params[:, d_action, :].squeeze().detach().cpu().numpy()
                print("action: ", d_action)
                print("c_action_params: ", c_action_params)
                
                
                # Step 3: Take action in the environment
                action = {
                'discrete': d_action,
                'continuous': c_action_params
                }
                ...
                # If the buffer has enough data, sample a batch for training
                if len(replay_buffer) >= batch_size:
                    ...
                    discrete_actions = torch.tensor([action["discrete"] for action in actions], dtype=torch.long)
                    continuous_params = torch.stack([torch.tensor(action["continuous"]) for action in actions])


                    # Step 4: Compute target Q-values for the batch
                    next_x_params = self.X_net(next_states)

                    next_q_values = self.Q_net(next_states, next_x_params)
                    target_q = rewards + self.gamma * torch.max(next_q_values, dim=1)[0] * (1 - dones.float())

                    # Step 5: Update Q(w) network using batch            
                    q_values_pred = self.Q_net(states, self.X_net(states)).gather(1, discrete_actions.view(-1, 1)).double()
                    q_loss = torch.nn.functional.mse_loss(q_values_pred.squeeze(), target_q)
                    print("target_q: ", target_q)
                    print("q_values_pred", q_values_pred)
                    self.Q_optimizer.zero_grad()
                    q_loss.backward(retain_graph=True)
                    self.Q_optimizer.step()
                    
                   
                    # Step 6: Update x(theta) network using batch
                    # For each state, compute the negative Q-value for the action chosen
                    x_loss = -q_values_pred.squeeze().mean()  # Negative Q-value for chosen actions
                    print(f"Episode {episode}, Step {len(replay_buffer)}, Q Loss: {q_loss.item()}, X Loss: {x_loss.item()}")
